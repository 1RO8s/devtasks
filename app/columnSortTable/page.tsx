...
function SortableColumnHeader({ group }: { group: ColumnGroup }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export default function ColumnSortTablePage() {
  ...
  // カスタムヘッダーを生成（ドラッグ可能なヘッダー）
  const headers = columnGroups.map((group) => (
    <SortableColumnHeader key={group.id} group={group} />
  ));

  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4">カラム並び替えサンプル（Tableコンポーネント使用）</h1>
      <p className="text-gray-600 mb-6">
        Tableコンポーネントを使用して実装したカラム並び替え機能です。
      ...
      >
        ...
        >
          <Table headers={headers} tableClassName="divide-y divide-gray-200">
            {sampleData.map((row) => (
              <Table.Row key={row.id}>
                {columnGroups.map((group) => (
                  <React.Fragment key={group.id}>
                    {/* Labelカラムのデータ（表示値） */}
                    <Table.Cell className="border-r border-gray-200">
